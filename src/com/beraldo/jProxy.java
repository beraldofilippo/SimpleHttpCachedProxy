...
public class jProxy extends Thread {
    ...
    private Cache<String, byte[]> httpCache;
    ...
    public jProxy(int port) {
        ...
        // Instantiate the cache with no automatic loader, I'll load data into cache when needed from each thread
        httpCache = Caffeine.newBuilder()
                .maximumWeight(5 * 1024 * 1024)
                .weigher((String resName, byte[] response) -> response.length)
                .recordStats()
                .build();
    }

    public jProxy(int port, int timeout) {
        thisPort = port;
        ptTimeout = timeout;
    ...
    public static void main(String args[]) {
        ...
        if (args.length == 0) {
            System.err.println("USAGE: java jProxy <port number>");
    ...
    public void run() {
        try {
            ...
            while (true) {
                ...
                ProxyThread t = new ProxyThread(client, httpCache);
...
class ProxyThread extends Thread {
    ...
    // Each of the Threads posseses a reference to the cache
    ...
    public ProxyThread(Socket s, Cache<String, byte[]> c) {
        pSocket = s;
        cache = c;
    }
        ...
        try {
            ...
            if (pos > 0) {
                try {
                    ...
                    if (debugLevel > 0)
                        debugOut.println("Error in ProxyThread, could not parse the request: " + e.getLocalizedMessage());
                }
                hostName = hostName.substring(0, pos);
            }

            byte[] finalRequest = request; // Lambdas require this thing
            String finalHostName = hostName; // Lambdas require this thing
            int finalHostPort = hostPort; // Lambdas require this thing

            // With this trick I single out the resource name which is being retrieved
            String req = new String(request);
            int idx = req.indexOf("GET");
            if(idx >= 0) { // The request is indeed a GET
                Scanner sc = new Scanner(req);
                sc.useDelimiter(" ");
                sc.next();
                String resourceName = sc.next();
                // Here the cache mechanism starts, looks for data with the given request and returns data accordingly
                // If there's a hit returns data immediately, otherwise it call a network request to be performed,
                // populating the cache as well
                System.out.println("### CACHE STATS ###");
                System.out.println("Looking for resource named: " + resourceName);
                response = cache.get(resourceName, s -> performNetworkRequest(finalRequest, finalHostName, finalHostPort));

                System.out.println("Hit rate: " + cache.stats().hitRate());
                System.out.println("Eviction count: " + cache.stats().evictionCount());
                System.out.println("### CACHE STATS ###");
            } else { // The request is not a GET, it's something else, perform the request regardless of the cache!
                response = performNetworkRequest(finalRequest, finalHostName, finalHostPort);
            }
            ...
            if (debugLevel > 0)
                debugOut.println("Error in ProxyThread: " + e.getLocalizedMessage());
            //e.printStackTrace();
        }
    }

    public byte[] performNetworkRequest(byte[] request, String hostName, int hostPort) {
        // the socket to the remote server
        Socket server = null;

        int requestLength = Array.getLength(request);
        ...
        try {
            if (server != null) {
                ...
                toReturn = getHTTPData(serverIn, true);
            ...
            if (debugLevel > 0)
                debugOut.println("Error in performNetworkRequest: " + e.getLocalizedMessage());
        }
        // This is going to provoke an anomaly, but it's nearly impossible to get to this point without
        // falling in a previous try/catch
        return null;
    }
